...
class Reflector extends Mesh {
  ...
  constructor(geometry?: BufferGeometry, options: ReflectorOptions = {}) {
    ...
    if (material.uniforms['tDiffuse']) material.uniforms['tDiffuse'].value = renderTarget.texture;
    if (material.uniforms['color']) material.uniforms['color'].value = color;
    if (material.uniforms['textureMatrix']) material.uniforms['textureMatrix'].value = textureMatrix;
    ...
    this.onBeforeRender = function (renderer, scene, camera) {
      ...
      renderer.state.buffers.depth.setMask(true); // make sure the depth buffer is writable
  ...
  static ReflectorShader = {
    ...
    fragmentShader: /* glsl */`
      ...
      varying vec4 vUv;
      uniform float opacity; // 修改：透明度uniform
...
export { Reflector as TransparentReflector };
